...
const HealthcareServiceStates  = {
    ...
    getHealthCareServiceByOrganization : async (parameters) => {
        ...
        let array = [];
        const {data: {entry: dataServiceType}} = await HealthcareServiceStates['getHealhcareService'](parameters);
        if(dataServiceType) {
...
export default function HealthcareService(action = null, params = null) {
    ...
    if (action) {
        const transformer = HealthcareServiceStates[action] ?? HealthcareServiceStates.__default__;
