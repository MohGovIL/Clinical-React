    ...
    try {
      if (params.prev) {
        return CRUDOperations(
          ...
          params.url +
            '?' +
            `date=le${params.date}&_count=1&_sort=date&patient=${params.patient}&status:not=arrived&status:not=booked&status:not=cancelled`,
        );
        ...
        return CRUDOperations(
          ...
          params.url +
            '?' +
            `date=ge${params.date}&_count=1&_sort=date&patient=${params.patient}&status:not=arrived&status:not=booked&status:not=cancelled`,
        );
