...
const appointmentsWithPatientsBasePath = (summary) =>
  `${fhirBasePath}/Appointment?status:not=arrived&_sort=priority,date,service-type${
    summary ? '&_summary=count' : '&_include=Appointment:patient'
  }`;

export const getAppointmentsWithPatients = (
  summary = false,
  date = '',
  organization = '',
  serviceType = '',
) => {
  return fhirTokenInstance().get(
    `${appointmentsWithPatientsBasePath(summary)}${
      date ? `&date=eq${date}` : ''
    }${
      organization
        ? `&actor:HealthcareService.organization=${organization}`
        : ''
...
export const updateAppointmentStatus = (appointmentId, value) => {
  return fhirTokenInstance().patch(
    `${fhirBasePath}/Appointment/${appointmentId}`,
    [
      {
        op: 'replace',
        path: '/status',
...
export const updatePatientData = (patientId, value) => {
  return fhirTokenInstance().patch(`${fhirBasePath}/Patient/${patientId}`, [
    ...
    { op: 'replace', path: '/name/0/given', value: [value.firstName, ''] },
    {
      op: 'replace',
      path: '/telecom/1',
      value: { system: 'email', value: value.patientEmail },
    },
    {
      op: 'replace',
      path: '/telecom/2',
      value: { system: 'phone', value: value.mobilePhone, use: 'mobile' },
    },
...
export const createNewEncounter = (appointment, facility) => {
  ...
  // });
  const reasonCode =
    appointment.examinationCode &&
    appointment.examinationCode.map((examination) => {
      return {
        coding: [
          {
            code: examination,
...
export const getHealhcareService = (organization) => {
  return fhirTokenInstance().get(
    `${fhirBasePath}/HealthcareService?organization=${organization}`,
  );
};

// const encountersWithPatientsBasePath = summary => '/Encounter?_include=Encounter:patient&_sort=date';
const encountersWithPatientsBasePath = (summary) =>
  `/Encounter?_sort=priority,date,service-type${
    summary ? '&_summary=count' : '&_include=Encounter:patient'
  }`;

export const getEncountersWithPatients = (
  summary = false,
  date = '',
  serviceProvider = '',
  serviceType = '',
  ...
  return fhirTokenInstance().get(
    `${fhirBasePath}${encountersWithPatientsBasePath(summary)}${
...
export const searchPatients = async (value) => {
  ...
  let mobileData = null;
  if (isNumeric(value)) {
    let identifierData = await fhirTokenInstance().get(
      `${fhirBasePath}${patientsFhirSeacrh}identifier:contains=${value}`,
    );
    let mobileData = await fhirTokenInstance().get(
      `${fhirBasePath}${patientsFhirSeacrh}mobile:contains=${value}`,
    );
    ...
    let byNameData = await fhirTokenInstance().get(
      `${fhirBasePath}${patientsFhirSeacrh}name=${value}`,
    );
    //for future Lexicographic search in ID open this
...
export const getHMO = (HmoId) =>
  fhirTokenInstance().get(`${fhirBasePath}/Organization/${HmoId}`);
...
export const getNextPrevAppointmentPerPatient = (date, patient, prev) => {
  ...
  try {
    if (prev) {
      return fhirTokenInstance().get(
        `${fhirBasePath}/Appointment?date=le${date}&_count=1&_sort=date&patient=${patient}&status:not=arrived&status:not=booked&status:not=cancelled`,
      );
    } else {
      return fhirTokenInstance().get(
        `${fhirBasePath}/Appointment?date=ge${date}&_count=1&_sort=date&patient=${patient}&status:not=arrived&status:not=booked&status:not=cancelled`,
      );
    }
  } catch (err) {
    console.log(err);
...
export const getCurrentEncounterPerPatient = (date, patient) => {
  ...
  try {
    return fhirTokenInstance().get(
      `${fhirBasePath}/Encounter?date=eq${date}&patient=${patient}`,
    );
  } catch (err) {
    console.log(err);
    return null;
...
export const getNextPrevEncounterPerPatient = (date, patient, prev) => {
  ...
  try {
    if (prev) {
      return fhirTokenInstance().get(
        `${fhirBasePath}/Encounter?date=le${date}&_count=1&_sort=-date&patient=${patient}`,
      );
    } else {
      return fhirTokenInstance().get(
        `${fhirBasePath}/Encounter?date=gt${date}&_count=1&_sort=-date&patient=${patient}`,
      );
    }
  } catch (err) {
    console.log(err);
...
export const getHealthCareServiceByOrganization = async (organizationId) => {
  ...
  } = await getHealhcareService(organizationId);
  if (dataServiceType) {
...
export const getAllEncounterDocuments = (encounterId, patientId, summary) => {
  return fhirTokenInstance().get(
    `${fhirBasePath}/DocumentReference?encounter=${encounterId}&patient=${patientId}${
...
export const getNextPrevAppointmentsPerPatient = (date, patient, prev) => {
  ...
  try {
    if (prev) {
      return fhirTokenInstance().get(
        `${fhirBasePath}/Appointment?date=le${date}&_sort=date&patient=${patient}&status:not=arrived&status:not=booked&status:not=cancelled`,
      );
    } else {
      return fhirTokenInstance().get(
        `${fhirBasePath}/Appointment?date=ge${date}&_sort=date&patient=${patient}&status:not=arrived&status:not=booked&status:not=cancelled`,
      );
    }
  } catch (err) {
    console.log(err);
...
export const getNextPrevEncountersPerPatient = (date, patient, prev) => {
  ...
  try {
    if (prev) {
      return fhirTokenInstance().get(
        `${fhirBasePath}/Encounter?date=le${date}&_sort=-date&patient=${patient}`,
      );
    } else {
      return fhirTokenInstance().get(
        `${fhirBasePath}/Encounter?date=gt${date}&_sort=-date&patient=${patient}`,
      );
    }
  } catch (err) {
    console.log(err);
